...
function createTranscriptOutputFile(Video $video, array $content): void
{
...
function cleanupTranscriptOutputDir(Video $video): void
{
    $outputDir = sys_get_temp_dir().'/whisperx_'.$video->id;
    if (is_dir($outputDir)) {
...
test('command runs transcription synchronously', function () {
    ...
    $this->artisan('app:transcribe-video', ['id' => $video->id])
        ->assertSuccessful();
...
test('command fails for non-existent sermon video', function () {
    $this->artisan('app:transcribe-video', ['id' => 999])
        ->assertFailed();
});
