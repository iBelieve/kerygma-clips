...
function makeVideo(int $segmentCount = 30): Video
{
    return Video::create([
...
test('createClip sets starts_at, ends_at, and duration from segment times', function () {
    $video = makeVideo(30); // segments: 0-5s, 5-10s, ... each 5s

    Livewire::test(EditVideo::class, ['record' => $video->id])
        ->call('createClip', 2, 5);

    $clip = VideoClip::where('video_id', $video->id)->sole();
...
test('createClip rejects when duration exceeds 90 seconds', function () {
    ...
    // 30 segments x 5s = 150s > 90s - should be rejected
...
function makeGappedVideo(int $segmentCount = 10, float $gap = 2.0): Video
{
    $stride = 4.0 + $gap;
...
test('createClip sets zero pause when segments are contiguous', function () {
    $video = makeVideo(30); // contiguous segments, no gaps
